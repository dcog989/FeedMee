...
const expanded = useExpandedFolders({
  get folders() {
    return navStore.folders;
  },
  get expandedFolders() {
    return navStore.expandedFolders;
  },
  set expandedFolders(v: Set<number>) {
    navStore.expandedFolders = v;
  },
  get autoCollapseFolders() {
    return navStore.settings.auto_collapse_folders;
  },
});
...
>
  ...
  <div class="footer-bar">
    ...
    <button type="button" class="footer-btn" onclick={openAddDialog} title="Manage Content" aria-label="Manage Content">
      <Rss size={18} />
    </button>
