...
pub fn get_folders_with_feeds(conn: &Connection) -> Result<Vec<Folder>> {
    ...
    let mut feed_stmt = conn.prepare("SELECT id, name, url, folder_id FROM feeds WHERE folder_id = ?1 ORDER BY name")?;

    let folders = folder_stmt.query_map([], |row| {
        let id: i64 = row.get(0)?;
        let name: String = row.get(1)?;

        let feeds = feed_stmt.query_map([id], |feed_row| {
            Ok(Feed {
                id: feed_row.get(0)?,
                name: feed_row.get(1)?,
                url: feed_row.get(2)?,
        ...
        Ok(Folder { id, name, feeds: feeds.unwrap_or_else(|_| vec![]) })
...
pub fn get_articles_for_feed(conn: &Connection, feed_id: i64) -> Result<Vec<Article>> {
    ...
    let articles = stmt.query_map([feed_id], |row| {
        Ok(Article {
            id: row.get(0)?,
            feed_id: row.get(1)?,
            title: row.get(2)?,
